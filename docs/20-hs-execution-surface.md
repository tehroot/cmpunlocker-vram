...
## Control passed — file-driven chains execute

```
RAW_BOOTER begin  n=1 sigSize=63488 wpr2=0x1ffffe00/0x00000000 EN=0x00000000
RAW_BOOTER iter=0 status=0xffff f754=0x00000001 f76c=0x00820040 EN=0x00000001
RAW_BOOTER end    EN=0x00000001
```

`f754`/`f76c` are echoed from the mapped buffer, so the payload that ran was
provably the file's. `EN_SW_OVERRIDE` went `0 -> 1` from a cold start.

**A payload generated by `recon/mkdmem.py` and dropped on disk executes as an HS
ROP chain.** Iteration no longer requires a rebuild.

## Correction: what this does and does not buy

An earlier section of this doc claimed the "one arbitrary write per Booter load"
ceiling shaped every experiment in [doc 19](19-a100-reference-diff.md), and that
N writes would retest the refusals. **That is wrong.**

`FUSE_OVR` already performed sequential writes within a single boot —
`EN_SW_OVERRIDE`, then `CTRL_OPT_NVDEC_DIS`, then optionally the gen fuses — each
its own Booter load, with `EN` persisting across them. The OPT refusals were
therefore already tested with the setup write in place. Write count per boot was
never the constraint.

What HS execution actually offers is **arbitrary code** instead of a fixed
single-PRI-write gadget:

- reads of priv-blocked space (`0x21000`, `0xbadf1100` from the host)
- read-modify-write, and sequences the stock chain cannot express
- anything depending on falcon-side state rather than PRI transactions

That is a real capability, and narrower than the earlier claim.

## The blocker for new chains

Designing a new chain needs gadget addresses, and the Booter image is encrypted
(entropy 7.93-7.96 across `0x0000-0x8000`). Static discovery is unavailable, so
the stock chain can be *replayed* with any `addr`/`value` but not yet *rewritten*.

Options, by evidence strength rather than appeal:

1. **Empirical gadget probing.** The harness makes each trial a file copy plus a
   modprobe cycle (~40 s). Vary one stack slot, observe write/hang/no-op. Fully
   automatable, but the search space is large and most trials will hang GSP.
2. **Reconsider the target.** The A100 diff pinned gen to two fuse bits, but 48
   fuses differ in total and the publish mask's source was never located. It may
   be reachable without touching fuses at all.
3. **Plaintext Booter from another source.** Older driver branches, or a variant
   that ships unencrypted. Not investigated.

Option 1 is the honest continuation of this thread. Option 2 is cheaper and has
not been exhausted.
