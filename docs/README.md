...
| [`13-gen3-synthesis.md`](13-gen3-synthesis.md) | **Gen3 synthesis** (7-agent forensic, post-Gen2-confirmed). Corrects doc 12 (`OPT_GEN3=1` captured; `PRIV_MISC_1` Gen3=[14:13]; advertise test is a valid *safe* gate). P(enforcement-only)≈35-40%, P(train on OcuLink)≈12%. Staged safe-first plan: Phase 0 read-only driver-context `0x85080` reachability dump → Phase 1 boot-time retrain-free advertise test → Phase 2 train on x16. Pivotal unknown: is `0x85080` writable from driver context (not just the poison-walled SEC2 ROP)? |
