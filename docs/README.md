...
| [`17-app08-orphaned-phy-routine.md`](17-app08-orphaned-phy-routine.md) | **Unreferenced link/PHY bring-up routine** in the signed 170HX `app08` (imem `0xcb30`–`0xcfaa`, 376 instrs). Six UPHY registers appear only in the 170HX ROM, all in `app08`. Neither the routine nor its `0x1411823c[11:10]==2` predicate is a branch/call target anywhere, and the image has zero indirect control flow — so Pry §6.4's "PHY calibration is never run" is **missing call sites**, not a runtime fuse check. All at `0x14118xxx`/`0x149axxxx`, outside the 16 MB BAR0 aperture, i.e. Falcon-only — which is why every host register sweep was in the wrong address space. With HS PC control the routine is invokable as-is, already signed |
